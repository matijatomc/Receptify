...
public partial class EditRecipePage : ContentPage
{
    ...
    private async Task LoadRecipeData()
    {
        ...
        var allKnownIngredients = await DatabaseService.GetAllIngredientsAsync();
        var steps = await DatabaseService.GetStepsByRecipeIdAsync(_recipeId);

        AllIngredients.Clear();
        foreach (var ing in allKnownIngredients)
        ...
        {
            var ing = allKnownIngredients.FirstOrDefault(i => i.Id == ri.IngredientId);
            if (ing != null)
            {
    ...
    private async void OnSaveClicked(object sender, EventArgs e)
    {
        ...
        if (!isValid)
        {
            await DisplayAlert("Greška", "Molimo ispuni obavezna polja.", "OK");
            return;
        }
        ...
        {
            await DatabaseService.AddRecipeIngredientAsync(new RecipeIngredient
            {
                RecipeId = _recipeId,
        ...
        await DisplayAlert("Uspjeh", "Recept je ažuriran.", "OK");
